...
<template>
  <div class="bottom-0 w-[100%] left-1/2 -translate-x-1/2">
    <div
      class="flex items-center justify-around gap-5 bg-[#fff] py-4"
    >
      <CategorySVG :mood="route.path == '/' ? '#448AF7':'#000'" @click="router.push('/')"/>
      <SearchSVG :mood="openSidebar ? '#448AF7' : '#000'" @click="openSidebar = true"/>
      <LocationSVG :mood="route.path == '/map' ? '#448AF7' : '#000'" @click="router.push('/map')"/>
    </div>
